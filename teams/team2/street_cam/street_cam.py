...
def main():
    ...
    EXAMPLES_BASE_DIR = '.'
    gn_mean = np.load(EXAMPLES_BASE_DIR + '/ilsvrc_2012_mean.npy').mean(1).mean(1)  # loading the mean file

    gn_labels_file = EXAMPLES_BASE_DIR + '/synset_words.txt'
    ...
    while True:
        ...
        center_x, center_y, width, height = overlay_on_image(display_image, filtered_objs)
        if center_x != -1:
            center_x = center_x / TY_NETWORK_IMAGE_WIDTH * actual_frame_width
            center_y = center_y / TY_NETWORK_IMAGE_HEIGHT * actual_frame_height
            width = width / TY_NETWORK_IMAGE_WIDTH * actual_frame_width
            height = height / TY_NETWORK_IMAGE_HEIGHT * actual_frame_height
            if width <= 200:
                need_advance += 1
                print(need_advance)
